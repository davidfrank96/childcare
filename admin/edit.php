...
if(!empty($_POST["submit"])) {
    $query = "UPDATE children set name = '".$_POST["name"]."', gender = '".$_POST["gender"]."', dob = '".$_POST["dob"]."' WHERE  id=".$_GET["id"];
...
$result = $db_handle->runQuery("SELECT * FROM children WHERE id='" . $_GET["id"] . "'");
?>
...
<form method="POST" action="" id="login-form" class="login-form" autocomplete="off" role="main" enctype="multipart/form-data">
  <div>


  <center><button>Update <?php echo $result[0]["name"];?>'s profile</button></center>
  </div>
...
<label class="label-email">
        ...
        <?php echo "<td><img class='g' src='../images/".$result[0]['image']."'</td>";?>
    </label>
    <label class="label-email">
      <input type="text" class="text" name="name"  placeholder="New Name"  tabindex="1" value="<?php echo $result[0]["name"]; ?>" required />
      <span class="required">Name</span>
    </label>

    <label class="label-email">
      <select name="gender" >
      <option style="" value="<?php echo $result[0]["gender"]; ?>"><?php echo $result[0]["gender"]; ?></option>
    ...
    <label class="label-email">
      <input type="date" class="text" name="dob" placeholder=""  tabindex="1"  value="<?php echo $result[0]["dob"]; ?>"  required />
...
<div class="email">
    <a href="carecoza.php">Go to dashboard</a>
