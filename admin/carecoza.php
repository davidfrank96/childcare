...
?>
  ...
  $sql = "SELECT children.dob, children.image, children.id, children.name, children.gender, parents.Pname,parents.phone   FROM children LEFT JOIN parents ON parents.Pid = children.Pid" . $queryCondition;
  $href = 'carecoza.php';          
...
<?php  
 $yr = date('Y');
  ...
  $sql3 =  mysqli_query($db, "SELECT COUNT(*)FROM children");   
  $sql4 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE gender = 'Male'");
  $sql5 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE gender = 'Female'");
  $sql6 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE YEAR(dob)  >= YEAR(NOW()) - 7");
  $sql7 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE YEAR(dob)  <= YEAR(NOW()) - 7");



$rs = mysqli_fetch_array($sql3);
$rm = mysqli_fetch_array($sql4);
$rg = mysqli_fetch_array($sql5);
$rt = mysqli_fetch_array($sql6);
$rr = mysqli_fetch_array($sql7);

echo '<h2 class="year" style: margin-left: 10px; display: inline; style="background-color: #aba6a6;" >Total Children = '. $rs[0].'</h2>';  ?>
<p>Total Male = <b><?php echo $rm[0];?></b> , Total Female = <b><?php  echo $rg[0];?></b> </p> 
<p class="year">Total Age (0yr - 7yrs) = <b><?php echo $rt[0];?></b> ,<hr> Total Age (7yrs and above) = <b><?php  echo $rr[0];?></b> </p>   



   
      <form name="frmSearch" method="post" action="carecoza.php" style="overflow-x: auto;">
      <div class="box">
      <p style="padding: 10px;"><input type="text" placeholder="Name" name="search[name]" class="demoInputBox" value="<?php echo $name; ?>" /><input type="text" placeholder="Surname" name="search[Pname]" class="demoInputBox" value="<?php echo $Pname; ?>" /><input type="submit" name="go" class="btnSearch" value="Search"><input type="reset" class="btnSearch" value="Reset" onclick="window.location ='carecoza.php'"></p>
      </div>
      ...
      <table cellpadding="5" cellspacing="1" style='width: 85%;'>
        <thead>
          ...
          <th class="table" style="padding: 10px;"><strong>Surname</strong></th>        
          <th class="table" style="padding: 10px;"><strong>Name</strong></th>
          <th class="table" style="padding: 10px;"><strong>Gender</strong></th>
          <th class="table" style="padding: 10px;"><strong>Parent Number</strong></th>
          <th class="table" style="padding: 10px;"><strong>Age</strong></th>
          <th class="table" style="padding: 10px;"><strong>Photo</strong></th>
          
          <th class="table" style="padding: 10px;"><strong>Action</strong></th>
...
?>          <td style="padding: 10px;"><?php echo $c; ?></td>
          ...
          <td>
          <a href="reg.php?id=<?php echo $result[$k]["id"]; ?>" title="Sign In" class="btnDeleteAction"> <img class="me"src="/c/images/icons/black/signin.png"></a>
           <a  href="edit.php?id=<?php echo $result[$k]["id"]; ?>"><img class="me"src="/c/images/icons/black/edit.png"></a>
           <a href="delete.php?action=delete&id=<?php echo $result[$k]["id"]; ?>" title="delete" class="btnDeleteAction" onclick="return confirm('Are you sure you want to delete this child?)"><img class="me"src="/c/images/icons/black/delete.png"></a>
          ...
          ?>
          <tr>
          <td style=" padding: 10px;" colspan="6" text-align=right> <?php echo $result["perpage"]; ?></td>
          </tr>
