...
?>
  ...
  $name = "";
  $Pname = ""; 
  $queryCondition = "WHERE MONTH(dob) = MONTH(NOW()) AND DAY(dob) = DAY(NOW()) ";
  ...
  $sql = "SELECT children.dob, children.image, children.id, children.name, children.gender, parents.Pname,parents.phone   FROM children INNER JOIN parents ON parents.Pid = children.Pid " . $queryCondition;
  $href = 'celeb.php';          
...
<?php  
  ...
  $sql6 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE MONTH(dob) = MONTH(NOW()) AND DAY(dob) = DAY(NOW()) ");
  $sql7 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE MONTH(dob) = MONTH(NOW()) AND DAY(dob)  = DAY(NOW()) + 7");

  // $sql7 =  mysqli_query($db, "SELECT COUNT(*)FROM children WHERE MONTH(dob) = MONTH(NOW()) AND DAY(dob)  = date_add(CURDATE(), interval 7 day");
...
<br>
      ...
      <table cellpadding="5" cellspacing="1" style='width: 85%;'>
        <thead>
          ...
          <th class="abs" style="padding: 10px;"><strong>Surname</strong></th>          
          <th class="abs" style="padding: 10px;"><strong>Name</strong></th>
          <th class="abs" style="padding: 10px;"><strong>Gender</strong></th>
          <th class="abs" style="padding: 10px;"><strong>Parent Number</strong></th>
          <th class="abs" style="padding: 10px;"><strong>Age</strong></th>
          <th class="abs" style="padding: 10px;"><strong>Photo</strong></th>
          
          <th class="abs" style="padding: 10px;"><strong>Action</strong></th>
...
?>          <td style="padding: 10px;"><?php echo $c; ?></td>
          ...
          <td>
          <a  href="tel:?id=<?php echo $result[$k]["phone"]; ?>" title="Call Parent" class="btnDeleteAction">Call Parents</a>
...
<article style="display: block;">
  <h2>Birthdays Coming In A Week Time!</h2>
     <?php

$sql2 = "SELECT children.dob, children.image, children.id, children.name, children.gender, parents.Pname,parents.phone   FROM children INNER JOIN parents ON parents.Pid = children.Pid WHERE MONTH(dob) = MONTH(NOW()) AND DAY(dob)  = DAY(NOW()) + 7 ";
    ...
    if(mysqli_num_rows($result2) > 0){
        ...
        while($row = mysqli_fetch_array($result2)){
            echo "<tr scope='row'>";
                ...
                echo "<td style='padding: 15px;'>" . $row['gender'] . "</td>";
                
                echo "<td style='padding: 15px;'>" . $row['phone'] . "</td>";?>

               
                <?php echo "<td style='padding: 15px;'><img class='g' src='../images/".$row['image']."'</td>";?>
          

        <td style='padding: 15px;'><a  href='tel:=<?php echo $row['phone']; ?>' title='Call Parent' class='btnDeleteAction'>Call Parents</a></td>
           <?php
