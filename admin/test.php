...
// $upd = mysqli_real_escape_string($db, $_POST['upd']);
  ...
  $sql1 =  mysqli_query($db, "INSERT INTO attendance
       SELECT *
       FROM children WHERE Pid = $cid");
...
<?php
 ...
 require_once("perpage.php");  
  ...
  $sql = "SELECT children.dob, children.image, children.id, children.name,children.Pid, children.gender, parents.Pname,parents.phone   FROM children INNER JOIN parents ON parents.Pid = children.Pid " . $queryCondition;
...
<div id="toys-grid" style="overflow-x: auto;">      
      <form name="frmSearch" method="post" action="test.php" style="overflow-x: auto;">
      <div class="box">
      <p style="padding: 10px;"><input type="text" placeholder="Name" name="search[name]" class="demoInputBox" value="<?php echo $name; ?>" /><input type="text" placeholder="Surname" name="search[Pname]" class="demoInputBox" value="<?php echo $Pname; ?>" /><input type="submit" name="go" class="btnSearch" value="Search"><input type="reset" class="btnSearch" value="Reset" onclick="window.location ='test.php'"></p>
      </div>
      
      <table>
        <thead>
          <tr>
          <th style="padding: 10px;"><strong>Position</strong></th>
          <th style="padding: 10px;"><strong>Surname Name</strong></th>          
          <th style="padding: 10px;"><strong>Child Name</strong></th>
          <th style="padding: 10px;"><strong>Gender</strong></th>
          <th style="padding: 10px;"><strong>Pic</strong></th>
...
$res1 = mysqli_query($db, "SELECT *  FROM attendance  WHERE id = 5");
  while ($row = mysqli_fetch_array($res1)) {
            ...
            if ( !empty($row['id'])){
              echo" <input style='font-size: 20px; position: relative; bottom: 40px:; background-color: purple; color: white;'  type='submit' name='call' Value ='Reg'>";
