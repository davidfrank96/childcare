...
<article style="height: 500px; width: 100%;" id="queue" >
...
<?php
 ...
 require_once("perpage.php");  
  ...
  $orderby = " ORDER BY Qid asc"; 
  $sql = "SELECT queue.Qid,attendance.image, attendance.id, attendance.name, attendance.gender, parents.Pname, parents.Pid  FROM attendance INNER JOIN parents ON attendance.Pid = parents.Pid INNER JOIN queue ON queue.Pid = parents.Pid " . $queryCondition;
  $href = 'que.php';          
...
<div id="toys-grid" style="overflow-x: auto;">      
      <form name="frmSearch" method="post" action="que.php" style="overflow-x: auto;">
      <div class="box" style="display: none;">
      <p style="padding: 10px;"><input type="text" placeholder="Name" name="search[name]" class="demoInputBox" value="<?php echo $name; ?>" /><input type="text" placeholder="Surname" name="search[Pname]" class="demoInputBox" value="<?php echo $Pname; ?>" /><input type="submit" name="go" class="btnSearch" value="Search"><input type="reset" class="btnSearch" value="Reset" onclick="window.location ='que.php'"></p>
      </div>
      
      <table>
        <thead>
          <tr>
          <th class="que" style="padding: 10px;"><strong>Position</strong></th>
          <th class="que" style="padding: 10px;"><strong>Surname Name</strong></th>          
          <th class="que" style="padding: 10px;"><strong>Child Name</strong></th>
          <th class="que" style="padding: 10px;"><strong>Gender</strong></th>
          <th class="que" style="padding: 10px;"><strong>Pic</strong></th>
        ...
        <tbody>
          ...
          <tr >
                ...
                <td style="padding: 10px;"><?php echo $result[$k]["Qid"]; ?></td>
...
echo"<form action='que.php' method='POST' >
                   <input type='hidden' name = 'cid' value='".$cid."'>
                   <input type='hidden' name = 'upd' value='1'>";
                   
$res1 = mysqli_query($db, "SELECT status  FROM queue  WHERE Pid = $cid");
        ...
        $stat =  $row['status'];
            ...
            if ( $stat == 1){
              ...
              echo "<h3>called</h3>";
                   ...
                   echo" <input style='font-size: 20px; position: relative; bottom: 40px:; background-color: purple; color: white;'  type='submit' name='call' Value ='Call In'>";
                 }
